...
public:
  ...
  PatchWorkpp(ros::NodeHandle *nh) : node_handle_(*nh) {
    ...
    node_handle_.param("adaptive_seed_selection_margin",
    ...
    pub_reject_pc = node_handle_.advertise<sensor_msgs::PointCloud2>(
        "reject_pc", 100, true);
    pub_normal =
        node_handle_.advertise<sensor_msgs::PointCloud2>("normals", 100, true);
    pub_noise =
        node_handle_.advertise<sensor_msgs::PointCloud2>("noise", 100, true);
    pub_vertical =
        node_handle_.advertise<sensor_msgs::PointCloud2>("vertical", 100, true);
...
private:
  ...
  int num_iter_;    // 每个patch进行平面拟合R-GRF的迭代次数
  int num_lpr_;     // 低点代表点的数量
  int num_min_pts_; // 每个patch中最少点的数量
  int num_zones_;   // CZM的zone数量
  int num_rings_of_interest_;

  double sensor_height_; // 传感器高度
  double th_seeds_;      // 地面种子高度阈值
  double th_dist_; // 点到平面的距离阈值，超过该值则认为非地面点
  double th_seeds_v_;      // 垂直面拟合（RVPF）阶段的种子阈值
  double th_dist_v_;       // 垂直面拟合（RVPF）阶段的距离阈值
  double max_range_;       // 最大检测范围
  double min_range_;       // 最小检测范围
  double uprightness_thr_; // 水平阈值
  double adaptive_seed_selection_margin_; // 自适应种子选择边距
  ...
  double RNR_ver_angle_thr_; // 反射噪声移除的垂直角度阈值
  double RNR_intensity_thr_; // 反射噪声移除的强度阈值

  bool verbose_;     // debug flag
  bool enable_RNR_;  // 是否启用反射噪声移除
  bool enable_RVPF_; // 是否启用垂直面拟合
  bool enable_TGR_;  // 是否启用时间地面恢复

  // 记录历史的平坦度和高度值，用于自适应阈值更新
  ...
  VectorXf normal_; // 平面法向量
  MatrixXf pnormal_;
  VectorXf singular_values_; // 奇异值
  Eigen::Matrix3f cov_;      // 协方差矩阵
  Eigen::Vector4f pc_mean_;  // 点云均值

  // For visualization
  bool visualize_;

  vector<int> num_sectors_each_zone_; // CZM中每个zone的sector数量，角度方向
  vector<int> num_rings_each_zone_; // CZM中每个zone的ring数量

  vector<double> sector_sizes_;  // 每个zone中sector的角度大小
  vector<double> ring_sizes_;    // 每个zone中ring的宽度大小
  vector<double> min_ranges_;    // 每个zone的最小半径
  vector<double> elevation_thr_; // 高度阈值
  vector<double> flatness_thr_;  // 平坦度阈值

  queue<int> noise_idxs_; // 反射噪声点索引队列

  vector<Zone> ConcentricZoneModel_; // CZM数据结构
...
// api: flush all patches in a zone
template <typename PointT>
...
// api: estimate plane from ground points using PCA
template <typename PointT>
inline void
...
// api: extract initial ground seeds from sorted pointcloud
template <typename PointT>
...
// api: extract initial ground seeds from sorted pointcloud，fixed th_seeds
template <typename PointT>
inline void PatchWorkpp<PointT>::extract_initial_seeds(
    const int zone_idx, const pcl::PointCloud<PointT> &p_sorted,
    pcl::PointCloud<PointT> &init_seeds) {
...
inline void PatchWorkpp<PointT>::estimate_ground(
  ...
  // step: 1 Reflected Noise Removal (RNR)
  if (enable_RNR_)
    reflected_noise_removal(cloud_in, cloud_nonground);

  t1 = ros::Time::now().toSec();

  // step: 2 Concentric Zone Model (CZM)
  ...
  for (int zone_idx = 0; zone_idx < num_zones_; ++zone_idx) {
    ...
    for (int ring_idx = 0; ring_idx < num_rings_each_zone_[zone_idx];
      ...
      for (int sector_idx = 0; sector_idx < num_sectors_each_zone_[zone_idx];
        ...
        // step: 3.2 Sort points in the patch based on z value
        ...
        // step: 3.3 Extract piecewise ground from the patch
      ...
      // step: 3.5 Temporal Ground Revert (TGR)
      if (!candidates.empty()) {
        if (enable_TGR_) {
...
// api: update elevation thresholds
template <typename PointT>
...
// api: update flatness thresholds
template <typename PointT>
...
// api: Temporal Ground Revert (TGR)
template <typename PointT>
...
// For adaptive
// api: extract piecewise ground from a patch
template <typename PointT>
inline void PatchWorkpp<PointT>::extract_piecewiseground(
  ...
  // step: 1 Region-wise Vertical Plane Fitting (R-VPF)
  ...
  // step: 2 Extract initial ground seeds
  extract_initial_seeds(zone_idx, src_wo_verticals, ground_pc_);
  estimate_plane(ground_pc_);
  ...
  // 3. Mainloop of ground plane fitting
...
// api: calculate mean and standard deviation of a vector
template <typename PointT>
...
PatchWorkpp<PointT>::pc2czm(const pcl::PointCloud<PointT> &src,
  ...
  for (int i = 0; i < src.size(); i++) {
    ...
    // step: 2 calculate radius & theta to determine which zone/ring/sector
    ...
    if ((r <= max_range_) && (r > min_range_)) {
      ...
      // cout << "[ CZM ] Point out of range: r = " << r << endl;
